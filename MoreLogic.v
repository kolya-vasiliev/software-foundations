...
Proof. intros X P H H2. inversion H2. apply H0. apply H. Qed.
...
Proof. intros ex_m X P H x. unfold excluded_middle in ex_m. 
  destruct (ex_m (P x)). 
    apply H0.
    apply ex_falso_quodlibet. apply H. exists x. apply H0. Qed.
...
Proof. intros X P Q. split. 
  intros H. inversion H as [x HH]. inversion HH.
    left. exists x. apply H0.
    right. exists x. apply H0.
  intros H. inversion H. 
    inversion H0 as [x HH]. exists x. left. apply HH.
    inversion H0 as [x HH]. exists x. right. apply HH. Qed.
...
Proof. intros X x1 x2 k1 k2 f. unfold override'. destruct (eq_nat_dec k1 k2).
...
|  all_0: all P []
|  all_l: forall x l, P x -> all P l -> all P (x::l).
...
Theorem forallb_spec: forall X (l:list X) test,
  forallb test l = true -> all (fun x => test x = true) l. 
Proof. intros X l test H. induction l. apply all_0.
  simpl in H. apply andb_prop in H. inversion H. 
  apply all_l. apply H0. apply IHl. apply H1. Qed.
...
| m_nil : merge [] [] []
| m_l1  : forall x l1 l2 l, merge l1 l2 l -> merge (x::l1) l2 (x::l)
| m_l2  : forall x l1 l2 l, merge l1 l2 l -> merge l1 (x::l2) (x::l).

Theorem filter_spec: forall X test (l:list X) l1 l2,
  merge l1 l2 l -> 
  all (fun x => test x = true) l1 -> 
  all (fun x => test x = false) l2 -> 
  filter test l = l1.
Proof. intros X test l. induction l.
  intros l1 l2 H H1 H2. inversion H. reflexivity.
  intros l1 l2 H H1 H2. simpl. destruct (test x) eqn: HH. 
   inversion H. 
     apply f_equal. rewrite <- H3, -> H0 in H1.
      inversion H1. apply IHl with l2. apply H5. apply H10. apply H2.
     rewrite <- H4, -> H0 in H2. inversion H2. rewrite H9 in HH. inversion HH.
   inversion H.
     rewrite <- H3, H0 in H1. inversion H1. rewrite H9 in HH. inversion HH.
    apply IHl with (l3). apply H5.  apply H1. rewrite <- H4 in H2. 
...
Theorem filter_longest: forall X test l' (l:list X), 
  subseq l' l -> 
  all (fun x => test x = true) l' -> 
  length l' <= length (filter test l).
Proof. intros X test l' l H H'. induction H. apply O_le_n.
  unfold filter. destruct (test x). 
    apply le_S. apply IHsubseq. apply H'.
    apply IHsubseq. apply H'.
  inversion H'. unfold filter. rewrite H2. simpl.
   apply n_le_m__Sn_le_Sm. apply IHsubseq. apply H3. Qed.
...
Proof. intros X xs ys x H. induction xs. right. apply H.
  inversion H. rewrite <- H1. left. apply ai_here.
    apply IHxs in H1. inversion H1. left. apply ai_later. apply H3.
      right. apply H3. Qed.

Lemma app_appears_in : forall (X:Type) (xs ys : list X) (x:X), 
     appears_in x xs \/ appears_in x ys -> appears_in x (xs ++ ys).
Proof. intros X xs ys x H. induction xs. 
  destruct H. inversion H. apply H.
  inversion H. 
    inversion H0. 
      apply ai_here.
...
Inductive disjoint {X} (xs ys : list X) : Prop :=
  | dj : (forall x, ~(appears_in x xs /\ appears_in x ys)) -> disjoint xs ys.
...
Inductive no_repeats {X} : list X -> Prop :=
  | nr_0  : no_repeats []
  | nr_SS : forall x l, ~ appears_in x l -> no_repeats l -> no_repeats (x::l). 
...
Theorem disjoint_no_repeats: forall X xs (ys:list X),
  no_repeats xs -> no_repeats ys -> disjoint xs ys -> no_repeats (xs ++ ys).
Proof. intros X xs ys. induction xs. intros H H1 H2. apply H1.
  intros H H1 H2. apply nr_SS. intros HH. apply appears_in_app in HH. 
    destruct HH. 
      inversion H. apply H5. apply H0.
      inversion H2. apply H3 with x. split. apply ai_here. apply H0.
    apply IHxs. 
      inversion H. apply H5.
      apply H1.
      inversion H2. apply dj. intros a H'. apply H0 with a. inversion H'. split.
...
Inductive nostutter:  list nat -> Prop :=
  ...
  | ns_SS  : forall y x xs, x <> y -> nostutter (x::xs) -> nostutter (y::x::xs).
...
Proof. intros X l1 l2. induction l1. reflexivity.
  simpl. apply f_equal. apply IHl1. Qed.

Lemma appears_in_app_split : forall (X:Type) (x:X) (l:list X),
  appears_in x l -> 
  exists l1, exists l2, l = l1 ++ (x::l2).
Proof. intros X x l H. induction l. inversion H.
  inversion H. exists []. exists l. reflexivity.
    apply IHl in H1. inversion H1 as [l1]. inversion H3 as [l2].
    exists (x0::l1). exists l2. simpl. apply f_equal. apply H4.
Qed.
