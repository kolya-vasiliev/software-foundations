...
Eval compute in (optimize_0plus (APlus (APlus (ANum 0)(ANum 0)) (ANum 1))).
...
Proof. intros b. induction b; simpl; 
...
Fixpoint optimize_0 (a:aexp) : aexp :=
  ...
  | ANum n =>
      ANum n
  | APlus e1 e2 =>
      match (optimize_0 e1) with
      | ANum 0 =>  (optimize_0 e2)
      | _ => APlus (optimize_0 e1) (optimize_0 e2)
      end
  | AMinus e1 e2 =>
      AMinus (optimize_0 e1) (optimize_0 e2)
  | AMult e1 e2 =>
      AMult (optimize_0 e1) (optimize_0 e2)
  end.

Eval compute in 
  (optimize_0 (AMinus (APlus (APlus (ANum 0) (ANum 0)) (ANum 0)) (ANum 0))).

Theorem optimize_0_sound: forall a,
  aeval (optimize_0 a) = aeval a.
Proof. induction a; 
  try (simpl; rewrite <- IHa1; rewrite <- IHa2); try reflexivity. 
  destruct (optimize_0 a1); try reflexivity. 
...
Theorem bevalR_beval: forall e b, bevalR e b <-> beval e = b.
Proof. intros. split.
  intros. induction H; 
    try constructor;
...
Proof. intros. destruct (eq_id_dec x y). 
...
Theorem update_eq : forall n x st,
  (update st x n) x = n.
Proof. intros. unfold update. apply eq_id.
Qed. 
...
Proof. intros. unfold update. apply neq_id. apply H. Qed.
...
Proof. intros.  unfold update. destruct (eq_id_dec x2 x1).
...
Proof. intros. unfold update. destruct (eq_id_dec x1 x2);
   subst; reflexivity.
Qed.
...
Proof. intros. unfold update. destruct (eq_id_dec x1 x3);
  destruct (eq_id_dec x2 x3); try reflexivity. 
