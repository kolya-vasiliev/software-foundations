...
Proof. intros n m. induction n as [|n'].
  ...
  Case "n = S n'".
    rewrite <- plus_n_Sm, <- IHn'. reflexivity. Qed.
...
Induction adds IH into the context.
...
Theorem plus_swap : forall n m p : nat, 
  n + (m + p) = m + (n + p).
Proof. intros n m p. 
  rewrite plus_assoc, plus_assoc, (plus_comm n m). reflexivity. Qed.
...
Theorem mult_nSm : forall n m : nat, 
  n * (S m) = n + n * m.
Proof. 
  intros n m. induction n as [|n']. 
  Case "n = 0".
    reflexivity.
  Case "n = S n'".
    simpl. rewrite IHn', plus_swap. reflexivity. Qed.
...
Proof. intros m n. induction m as [|m'].
  ...
  Case "m = S m'".
    rewrite mult_nSm, <- IHm'. reflexivity. Qed.
