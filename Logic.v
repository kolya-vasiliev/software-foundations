...
Theorem or_distributes_over_and_2 : forall P Q R : Prop,
  (P \/ Q) /\ (P \/ R) -> P \/ (Q /\ R).
Proof. intros P Q R H. destruct H as [[HP | HQ] [HP' | HR]].
  left. apply HP.
  left. apply HP.
  left. apply HP'.
...
Theorem contrapositive : forall P Q : Prop,
  (P -> Q) -> (~Q -> ~P).
Proof. intros P Q H. unfold not. intros NQ p. 
  apply NQ. apply H. apply p. Qed.
...
Theorem not_both_true_and_false : forall P : Prop,
  ~ (P /\ ~P).
Proof. unfold not. intros P H. destruct H as [HP HNP]. 
...
Proof. intros P. 
  apply contrapositive with (P:= ~ (P \/ ~P)) (Q:= (~P /\ ~~P)).
    intros H. split. intros p. apply H. left. apply p.
    intros p. apply H. right. apply p. 
  apply not_both_true_and_false. Qed.
...
Proof. intros peirce P NNp. apply (peirce P False). 
  intros Np. apply NNp in Np. inversion Np. Qed.

Theorem classic_implies_excluded_middle: classic -> excluded_middle.
Proof. intros classic P. apply classic. 
...
Proof. intros excluded_middle P Q. destruct (excluded_middle P).
  left. apply H. destruct (excluded_middle Q).
  right. apply H0. intros H'. 
  assert (H'': (~ P /\ ~ Q)).
    apply conj. apply H. apply H0.
  apply H' in H''. inversion H''.
Qed.

Theorem de_morgan_not_and_not_implies_implies_to_or:
  de_morgan_not_and_not -> implies_to_or.
Proof. intros de_morgan P Q H. apply de_morgan.
  intros H'. destruct H' as [NNp Nq].
  apply NNp. intros p. apply Nq. apply H. apply p. Qed.

Theorem implies_to_or_implies_peirce:
  implies_to_or -> peirce.
Proof. intros impl P Q PQP.
  destruct (impl P P). intros p. apply p.
    apply PQP. intros p. apply H in p. inversion p.
    apply H.
Qed.
...
Theorem false_beq_nat : forall n m : nat,
     n <> m ->
     beq_nat n m = false.
Proof. intros n m H. destruct (beq_nat n m) eqn: H'.
  apply ex_falso_quodlibet. apply H. apply beq_nat_true. apply H'.
  reflexivity.
Qed.
(** [] *)
...
Proof. intros n m H1 H2. rewrite H2 in H1. 
  rewrite <- beq_nat_refl in H1. inversion H1. Qed. 
