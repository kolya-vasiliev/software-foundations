...
Proof. intros c st st'; split; intros H. 
  ...
  apply E_Seq with st'. assumption. constructor. Qed.
...
Proof. split; intros. 
  inversion H0. 
    ...
    assumption.
  apply E_IfFalse. apply H. assumption. Qed.
...
Proof. split; intros; inversion H; subst;
 ...
 try (simpl in H5; 
    apply f_equal  with (f:=negb) in H5; rewrite negb_involutive in H5; 
    apply H5). 
Qed.
...
Proof. split; intros;
  [inversion H; inversion H2 | inversion H; inversion H5]; subst; 
...
Theorem assign_aequiv : forall X e,
  ...
  cequiv SKIP (X ::= e).
Proof. intros.
  assert (H': forall st', update st' X (aeval st' e) = st').
    intros. replace (aeval st' e) with (st' X).
    apply functional_extensionality. intro. 
    rewrite update_same; reflexivity.
    apply H.
  split; intros. inversion H0; subst. 
  rewrite <- H' at 2. constructor. reflexivity.
  inversion H0; subst.  
  rewrite (H' st). constructor.
Qed.
