...
Definition even_X : com :=
  ...
  END;;
  IFB BEq (ANum 0) (AId X) THEN Z::= (ANum 0) ELSE (Z::= (ANum 1)) FI). 

Example test_even_X : 
  test_ceval (update empty_state X 18) even_X
  = Some (0, 0, 0).
Proof.  reflexivity. Qed.
...
Proof. 
  ...
  inversion IHHce1 as [i1 He]. inversion IHHce2 as [i2 He2].
  exists (S (i1+i2)). simpl. 
    assert (ceval_step st c1 (i1 + i2) = Some st').
      apply ceval_step_more with i1. omega. assumption.
    assert (ceval_step st' c2 (i1 + i2) = Some st'').
      apply ceval_step_more with i2. omega. assumption.
    rewrite H, H0. reflexivity.
  
  inversion IHHce as [i He]. exists (S i). simpl. rewrite H, He. reflexivity.
  inversion IHHce as [i He]. exists (S i). simpl. rewrite H, He. reflexivity.
