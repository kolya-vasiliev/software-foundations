...
Proof.
  ...
  unfold lt. unfold transitive.
  intros n m o Hnm Hmo.
  induction Hmo as [| o' Ho'o].
  apply le_S. apply Hnm.
  apply le_S. apply IHHo'o.
Qed.
(** [] *)
...
Theorem le_S_n : forall n m,
  (S n <= S m) -> (n <= m).
Proof. intros. inversion H. apply le_n.
...
Theorem le_Sn_n : forall n,
  ~ (S n <= n).
Proof. intros n H. induction n. inversion H.
  inversion H. 
    assert (forall n, (~(S n = n))).
      intros k. intros HH. induction k. inversion HH.
      apply IHk. inversion HH. apply HH.
    apply (H0 n). apply H1.
  apply IHn. apply le_trans with (S (S n)).
...
Proof. intros a. induction a.
  intros b H H'. inversion H'. reflexivity.
  intros b H H'. inversion H as [|b']. reflexivity.
  apply f_equal. apply IHa. apply le_S_n. rewrite <- H1 in H. apply H.  
  apply le_S_n. rewrite <- H1 in H'. apply H'.
Qed. 
...
Theorem le_step : forall n m p,
  ...
  m <= S p ->
  n <= p.
Proof. unfold lt. intros. apply Sn_le_Sm__n_le_m. 
  apply le_trans with (m). apply H. apply H0. Qed. 
...
Proof. intros. induction H.
  apply H0. apply rsc_step with y. apply H. 
  apply IHrefl_step_closure.  apply H0. Qed.
...
Proof. intros X R x y. split.
  intros H. induction H. 
    apply rsc_R. apply H.
    apply rsc_refl. apply rsc_trans with y.
    apply IHclos_refl_trans1. apply IHclos_refl_trans2.
  intros H. induction H.
    apply rt_refl.
    apply rt_trans with y. apply rt_step. apply H. 
