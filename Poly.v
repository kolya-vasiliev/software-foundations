...
Proof. intros X a l. induction l as [|n l'].
  reflexivity.
...
Theorem rev_involutive : forall X : Type, forall l : list X,
  rev (rev l) = l.
Proof. intros X l. induction l as [|n l'].
  reflexivity.
...
Proof. intros X l1 l2 v. induction l1 as [|n l'].
  reflexivity.
  simpl. rewrite IHl'. reflexivity. Qed.
...
Fixpoint split
  ...
  match l with
  | [] => ([],[])
  | (x, y) :: l' => let s := split l' in (x:: fst(s), y :: snd(s)) 
  end.
...
Theorem snoc_app: forall X x (l:list X),
  snoc l x = l ++ [x].
Proof. intros X x l. induction l as [|n l'].
  reflexivity.
  simpl. rewrite IHl'. reflexivity. Qed.

Theorem map_app: forall X Y (f:X->Y) (l1 l2:list X),
  map f (l1 ++ l2) = map f l1 ++ map f l2.
Proof. intros X Y f l1 l2. induction l1 as [|n l'].
  reflexivity.
  simpl. rewrite IHl'. reflexivity. Qed.
...
Proof. intros X Y f l. induction l as [|n l'].
  reflexivity.
  simpl. rewrite <- IHl', snoc_app, snoc_app, map_app. reflexivity. Qed.
...
Fixpoint map' (X Y:Type) (f:X->Y) (l:list X)
  ...
  match l with
  | nil      => @nil Y 
  | cons h t => @cons Y (f h) (map' X Y f t)
  end.
...
Fixpoint filter' (X:Type) (test: X->bool) (l:list X)
  ...
  match l with
  | nil     => @nil X
  | cons h t => if test h then @cons X h (filter' X test t)
...
Definition forany {X} (test:X -> bool) (l:list X) :=
  fold (fun x y => orb (test x) y) l false.

Example forany_example: true = forany (fun n => beq_nat 7 n) [4; 7; 1; 5].
Proof. reflexivity. Qed.
...
(* FILL IN HERE *)
*)
Theorem length_index: forall X l n, 
  length l = n -> @index X n l = None.
Proof. intros X l. 
  induction l as [|h l'].
  intros n H. rewrite <- H. reflexivity. 
  intros n H. rewrite <- (IHl' (pred n)), <- H. reflexivity. 
