...
Theorem silly_ex : 
     ...
     evenb 3 = true ->
     oddb 4 = true.
Proof. intros H1 H2. apply H1, H2. Qed.
...
Proof. intros n H. simpl in H. destruct n. 
  reflexivity.
  inversion H. Qed.
...
Proof.
  ...
  intros m H. destruct m.
    ...
    apply f_equal, IHn'. inversion H.
    inversion H. apply IHn' in H1. inversion H1. reflexivity. 
Qed.
...
Proof. intros n. induction n as [|n'].
  ...
  intros m H. destruct m.
    inversion H.
    apply IHn' in H. inversion H. reflexivity.
Qed.
...
Theorem index_after_last: forall (n : nat) (X : Type) (l : list X),
     length l = n ->
     index n l = None.
Proof. intros n X l. generalize dependent n. induction l as [|h l'].
  reflexivity.
...
Theorem length_snoc''' : forall (n : nat) (X : Type) 
     ...
     length l = n ->
     length (snoc l v) = S n. 
Proof. intros n X v l. generalize dependent n. induction l as [|h l'].
...
Theorem app_length_cons : forall (X : Type) (l1 l2 : list X) 
     ...
     length (l1 ++ (x :: l2)) = n ->
     S (length (l1 ++ l2)) = n.
Proof. intros X l1 l2 x. induction l1 as [| h l'].
  simpl. intros n H. apply H.
  intros n H. destruct n.
    inversion H.
    apply f_equal. apply IHl'. inversion H. reflexivity. Qed.
...
Proof. intros X n l. generalize dependent n. 
  induction l as [|h l'].
    ...
    intros n H. destruct n.
      inversion H.
      rewrite <- plus_n_Sm. simpl. apply f_equal.
      inversion H. rewrite H1. apply IHl' in H1. 
      rewrite <- H1. symmetry. 
      apply app_length_cons with (x:=h). reflexivity. Qed.
...
Theorem double_induction: forall (P : nat -> nat -> Prop), 
  ...
  forall m n, P m n. 
Proof. intros P H0 H1 H2 H3 m. induction m.
  induction n. 
    apply H0. 
...
Proof. intros X x1 x2 k1 k2 f. unfold override. 
...
Proof. induction l as [|h l'].
  ...
  intros l1 l2 H. destruct h. inversion H. 
  simpl. apply f_equal. apply IHl'. destruct (split l'). reflexivity. 
Qed.
(** [] *)
...
Proof. intros X x1 k1 k2 f H. unfold override.
  destruct (beq_nat k1 k2) eqn:H'.
    rewrite <- H. apply f_equal. apply beq_nat_true. apply H'.
    reflexivity.
Qed.
...
Theorem beq_nat_trans : forall n m p,
  ...
  beq_nat m p = true ->
  beq_nat n p = true.
Proof. intros n m p eq1 eq2.
  assert (H: n = p).
    apply trans_eq with m.
    apply beq_nat_true. apply eq1. 
    apply beq_nat_true. apply eq2.
  rewrite H. symmetry. apply beq_nat_refl. Qed.
(** [] *)
...
Proof. intros X x1 x2 k1 k2 k3 f H. unfold override. 
  destruct (beq_nat k1 k3) eqn: H1.
    destruct (beq_nat k2 k3) eqn: H2.
      rewrite beq_nat_sym in H1.
      rewrite beq_nat_trans with (m:=k3) in H. 
        inversion H. apply H2. apply  H1.
      reflexivity.
    reflexivity.
Qed.
...
Theorem filter_exercise : forall (X : Type) (test : X -> bool)
     ...
     filter test l = x :: lf ->
     test x = true.
Proof. intros X test x l lf H. induction l.
  inversion H. simpl in H. 
  destruct (test x0) eqn:H'.
    inversion H. rewrite <- H1. apply H'.
    apply IHl. apply H.
Qed.
...
Fixpoint forallb {X} (test:X -> bool) (l:list X) :=
  match l with 
  | [] => true
  | h :: t => andb (test h) (forallb test t)
  end.

Fixpoint existsb {X} (test:X -> bool) (l:list X) :=
  match l with 
  | [] => false
...
Proof. induction l. 
  reflexivity. simpl. 
  rewrite IHl. unfold existsb'. simpl. destruct (test x). 
